/**
 * tests/unit/mcp-build.test.js
 * Structural tests for src/features/mcp-build.ts
 * Run: node tests/unit/mcp-build.test.js
 */
'use strict';
...
const OUT = path.join(__dirname, '../../out-test/features/mcp-build.js');
...
Module._load = function(req, parent, isMain) {
    if (req === 'vscode') {
        return {
            ...
            window:    { showErrorMessage() {}, showInformationMessage() {}, createOutputChannel: () => ({ appendLine() {}, show() {}, dispose() {} }), createTerminal: () => ({ sendText() {}, show() {}, dispose() {} }) },
...
test('source file has copyright header', () => {
    const src = path.join(__dirname, '../../src/features/mcp-build.ts');
    if (!fs.existsSync(src)) return;
    assert.ok(fs.readFileSync(src, 'utf8').includes('CieloVista'));
});
test('activate registers mcp.build command', () => {
    registered.clear();
    mod.activate({ subscriptions: [] });
    const keys = [...registered.keys()];
    assert.ok(keys.some(k => k.includes('mcp') || k.includes('build')), `Expected mcp/build command, got: ${keys.join(', ')}`);
});
