/**
 * tests/unit/html-template-downloader.test.js
 * Structural tests for src/features/html-template-downloader.ts
 * Run: node tests/unit/html-template-downloader.test.js
 */
'use strict';
...
const OUT = path.join(__dirname, '../../out-test/features/html-template-downloader.js');
...
Module._load = function(req, parent, isMain) {
    if (req === 'vscode') {
        return {
            ...
            window:    { showErrorMessage() {}, showInformationMessage() {}, showInputBox: async () => undefined, createOutputChannel: () => ({ appendLine() {}, show() {}, dispose() {} }) },
...
console.log('\nhtml-template-downloader unit tests\n' + '─'.repeat(50));
...
test('source file has copyright header', () => {
    const src = path.join(__dirname, '../../src/features/html-template-downloader.ts');
