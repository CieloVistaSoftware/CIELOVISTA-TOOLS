/**
 * run-regression-tests.js
 *
 * All tests run concurrently — results stream in as each completes.
 * File-based tests in tests/regression/REG-*.test.js are auto-discovered;
 * no manual wiring is needed when adding a new regression test file.
 *
 * Run directly:  node scripts/run-regression-tests.js
 * Run via npm:   npm run test:regression
 */
'use strict';

const fs   = require('fs');
const path = require('path');
const { spawn } = require('child_process');

const ROOT = path.resolve(__dirname, '..');
const SRC  = path.join(ROOT, 'src');

// ── Helpers ───────────────────────────────────────────────────────────────────

function readJson(p) {
  return JSON.parse(fs.readFileSync(p, 'utf8'));
}

function walkTs(dir, results = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const full = path.join(dir, entry.name);
    if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== '.git') {
      walkTs(full, results);
    } else if (entry.isFile() && (entry.name.endsWith('.ts') || entry.name.endsWith('.js'))) {
      results.push(full);
    }
  }
  return results;
}

function srcContents() {
  return walkTs(SRC).map(f => ({ file: path.relative(ROOT, f), src: fs.readFileSync(f, 'utf8') }));
}

function stripTemplateLiterals(src) {
  return src.replace(/`(?:[^`\\]|\\.)*`/gs, '``');
}

// ── Result tracking ───────────────────────────────────────────────────────────

let passed = 0, failed = 0;
const failures = [];

function recordPass(id, name) {
  passed++;
  console.log(`  ✓ ${id}: ${name}`);
}

function recordFail(id, name, message) {
  failed++;
  failures.push({ id, name, message });
  console.error(`  ✗ ${id}: ${name}`);
  const lines = message.split('\n');
  console.error(`    → ${lines[0]}`);
  lines.slice(1, 8).forEach(l => console.error(`      ${l}`));
}

// ── Test factories ────────────────────────────────────────────────────────────

// Inline check — synchronous logic wrapped in a Promise so it joins Promise.all.
function check(id, name, fn) {
  return Promise.resolve().then(() => {
    try { fn(); recordPass(id, name); }
    catch (e) { recordFail(id, name, e.message); }
  });
}

function assert(condition, message) {
  if (!condition) throw new Error(message);
}

// Spawn a Node.js script as a non-blocking child process.
function subprocess(id, name, scriptPath, args = []) {
  return new Promise(resolve => {
    let out = '';
    const child = spawn(process.execPath, [scriptPath, ...args], { cwd: ROOT });
    child.stdout.on('data', d => { out += d; });
    child.stderr.on('data', d => { out += d; });
    child.on('close', code => {
      if (code === 0) { recordPass(id, name); }
      else { recordFail(id, name, out.trim().slice(0, 1000)); }
      resolve();
    });
  });
}

// Spawn an arbitrary executable (for tsc, etc.).
// On Windows, .bin scripts are .cmd wrappers — run via cmd.exe /c.
function command(id, name, exe, args = [], cwd = ROOT) {
  const cmdExe   = exe.endsWith('.cmd') ? exe : (process.platform === 'win32' ? exe + '.cmd' : exe);
  const useShell = process.platform === 'win32';
  const spawnExe  = useShell ? 'cmd.exe' : exe;
  const spawnArgs = useShell ? ['/c', cmdExe, ...args] : args;
  return new Promise(resolve => {
    let out = '';
    const child = spawn(spawnExe, spawnArgs, { cwd, shell: false });
    child.stdout.on('data', d => { out += d; });
    child.stderr.on('data', d => { out += d; });
    child.on('close', code => {
      if (code === 0) { recordPass(id, name); }
      else { recordFail(id, name, out.trim().slice(0, 800)); }
      resolve();
    });
  });
}

// Auto-discovered file-based regression test.
function regressionFile(file) {
  const fullPath = path.join(ROOT, 'tests', 'regression', file);
  const id   = (file.match(/^(REG-\d+)/) ?? [, '?'])[1];
  const name = file.replace(/^REG-\d+-?/, '').replace(/\.test\.js$/, '').replace(/-/g, ' ');
  return subprocess(id, name, fullPath);
}

// ── Main ──────────────────────────────────────────────────────────────────────

async function main() {
  console.log('\nCieloVista Tools — Regression Test Suite');
  console.log('─'.repeat(50));
  console.log('  Launching all tests concurrently...\n');

  const all = [];

  // REG-001a: Every package imported in extension host src/ must be in dependencies.
  all.push(check('REG-001a', 'Every package imported in src/ is listed in dependencies', () => {
    const BUILTINS = new Set([
      'fs','path','os','child_process','util','crypto','stream','events',
      'url','http','https','net','tls','zlib','buffer','assert','process',
      'querystring','readline','string_decoder','timers','vm',
    ]);
    const pkg  = readJson(path.join(ROOT, 'package.json'));
    const deps = new Set(Object.keys(pkg.dependencies ?? {}));
    const violations = [];
    for (const { file, src } of srcContents()) {
      if (file.includes('scripts') || file.includes('tests') || file.includes('mcp-server')) { continue; }
      const stripped = stripTemplateLiterals(src);
      const importRe = /^import\s+(?:[\s\S]*?\s+from\s+)?['"]([^'"./][^'"]*)['"]/gm;
      let m;
      while ((m = importRe.exec(stripped)) !== null) {
        const mod = m[1].split('/')[0];
        if (mod === 'vscode' || BUILTINS.has(mod) || mod.startsWith('@types/')) { continue; }
        if (!deps.has(mod)) {
          violations.push(`  ${file}\n    imports '${m[1]}' but '${mod}' is not in dependencies`);
        }
      }
    }
    assert(violations.length === 0,
      `Packages imported in src/ are MISSING from package.json "dependencies".\n` +
      `These will cause "Cannot find module" crash on activation (see REG-011):\n\n` +
      `${violations.join('\n')}\n\n` +
      `FIX: Move each from devDependencies to dependencies, add !node_modules/<pkg>/** to .vscodeignore.`);
  }));

  // REG-001b: The "package" script must NOT contain --no-dependencies.
  all.push(check('REG-001b', 'vsce package script does not use --no-dependencies flag', () => {
    const pkg    = readJson(path.join(ROOT, 'package.json'));
    const script = pkg.scripts?.package ?? '';
    assert(!script.includes('--no-dependencies'),
      `The "package" npm script contains --no-dependencies — strips ALL node_modules from the VSIX.\n` +
      `FIX: Change the package script to: "package": "vsce package"`);
  }));

  // REG-001c: No require() of external packages in extension host compiled output.
  all.push(check('REG-001c', 'No require() of external packages in extension host', () => {
    const BUILTINS = new Set([
      'fs','path','os','child_process','util','crypto','stream','events',
      'url','http','https','net','tls','zlib','buffer','assert','process',
      'querystring','readline','string_decoder','timers','vm',
    ]);
    const violations = [];
    for (const { file, src } of srcContents()) {
      if (file.includes('scripts') || file.includes('tests') || file.includes('mcp-server')) { continue; }
      const stripped = stripTemplateLiterals(src);
      const re = /require\(['"]([^'"./][^'"]*)['"]\)/g;
      let m;
      while ((m = re.exec(stripped)) !== null) {
        const mod = m[1].split('/')[0];
        if (!BUILTINS.has(mod) && mod !== 'vscode') {
          violations.push(`  ${file}: require('${m[1]}')`);
        }
      }
    }
    assert(violations.length === 0,
      `Extension host uses require() for external packages — crash on activation:\n\n` +
      `${violations.join('\n')}\n\nFIX: Replace require() with ES import.`);
  }));

  // REG-002: logError interface compliance (subprocess — avoids blocking the event loop).
  all.push(subprocess('REG-002', 'All logError callers match their interface contract',
    path.join(ROOT, 'scripts', 'test-logerror-interface.js')));

  // REG-003: TypeScript compiles clean (spawn tsc — avoids blocking the event loop).
  const tscCandidates = [
    path.join(ROOT, 'node_modules', '.bin', 'tsc'),
    path.join(ROOT, '..', '..', '..', 'node_modules', '.bin', 'tsc'),
  ];
  const tsc = tscCandidates.find(p => fs.existsSync(p));
  if (tsc) {
    all.push(command('REG-003', 'TypeScript compiles with zero errors', tsc, ['--noEmit'], ROOT));
  } else {
    all.push(check('REG-003', 'TypeScript compiles with zero errors', () => {
      assert(false, 'tsc not found — run npm install in the project root');
    }));
  }

  // REG-004: Every catalog command ID must be wired through registerCommand().
  all.push(check('REG-004', 'Every catalog command ID is wired through a registerCommand() call', () => {
    const catalogPath = path.join(SRC, 'features', 'cvs-command-launcher', 'catalog.ts');
    const catalogSrc  = fs.readFileSync(catalogPath, 'utf8');
    const catalogIds  = [...catalogSrc.matchAll(/id:\s*'([^']+)'/g)].map(m => m[1]);
    const registeredIds = new Set();
    for (const file of walkTs(SRC)) {
      if (file.endsWith('catalog.ts')) { continue; }
      const src = fs.readFileSync(file, 'utf8');
      for (const m of src.matchAll(/registerCommand\s*\(\s*['"]([^'"]+)['"]/g)) {
        registeredIds.add(m[1]);
      }
      for (const m of src.matchAll(/registerCommand\s*\(\s*([A-Za-z_$][\w$]*)\s*,/g)) {
        const varName = m[1];
        if (['async', 'await', 'function', 'return'].includes(varName)) { continue; }
        const declRe = new RegExp(`(?:const|let|var)\\s+${varName}\\b[^=;\\n]*=\\s*['"]([^'"]+)['"]`);
        const decl = declRe.exec(src);
        if (decl) { registeredIds.add(decl[1]); }
      }
      for (const m of src.matchAll(/registerFixed\s*\(\s*[^,]+,\s*['"]([^'"]+)['"]/g)) {
        registeredIds.add(m[1]);
      }
    }
    const missing = catalogIds.filter(id => !registeredIds.has(id));
    assert(missing.length === 0,
      `${missing.length} catalog ID(s) have NO matching registerCommand() call:\n  ${missing.join('\n  ')}`);
  }));

  // REG-005: No duplicate command IDs in catalog.
  all.push(check('REG-005', 'No duplicate command IDs in catalog.ts', () => {
    const catalogSrc = fs.readFileSync(path.join(SRC, 'features', 'cvs-command-launcher', 'catalog.ts'), 'utf8');
    const ids = [...catalogSrc.matchAll(/id:\s*'([^']+)'/g)].map(m => m[1]);
    const seen = new Set(), dupes = [];
    for (const id of ids) { if (seen.has(id)) { dupes.push(id); } else { seen.add(id); } }
    assert(dupes.length === 0, `Duplicate command IDs in catalog: ${dupes.join(', ')}`);
  }));

  // REG-006: package.json contributes.commands covers all catalog IDs.
  all.push(check('REG-006', 'package.json contributes.commands covers all catalog IDs', () => {
    const pkg = readJson(path.join(ROOT, 'package.json'));
    const contributed = new Set((pkg.contributes?.commands ?? []).map(c => c.command));
    const catalogSrc  = fs.readFileSync(path.join(SRC, 'features', 'cvs-command-launcher', 'catalog.ts'), 'utf8');
    const catalogIds  = [...catalogSrc.matchAll(/id:\s*'([^']+)'/g)].map(m => m[1]);
    const missing = catalogIds.filter(id => !contributed.has(id));
    assert(missing.length === 0,
      `${missing.length} catalog ID(s) missing from package.json contributes.commands:\n  ${missing.join('\n  ')}`);
  }));

  // REG-007: extension.ts activates all feature modules.
  all.push(check('REG-007', 'extension.ts activates all feature modules', () => {
    const extSrc = fs.readFileSync(path.join(SRC, 'extension.ts'), 'utf8');
    const imported = [...extSrc.matchAll(/(?<!de)activate\s+as\s+(\w+)/g)].map(m => m[1]);
    const missing = imported.filter(alias => {
      const called  = new RegExp('\\b' + alias + '\\s*\\(context\\)').test(extSrc);
      const enabled = new RegExp('activateIfEnabled\\([^)]+,\\s*' + alias + '\\s*,').test(extSrc);
      return !called && !enabled;
    });
    assert(missing.length === 0,
      missing.length + ' feature activate() functions imported but NEVER called in extension.ts:\n  ' +
      missing.join('\n  '));
  }));

  // REG-008: No bare console.log (warning only).
  all.push(check('REG-008', 'No bare console.log in extension host (warning)', () => {
    const violations = [];
    for (const { file, src } of srcContents()) {
      if (file.includes('scripts') || file.includes('tests') || file.includes('mcp-server')) { continue; }
      src.split('\n').forEach((line, i) => {
        if (/console\.(log|warn|error)\s*\(/.test(line) && !line.trim().startsWith('//')) {
          violations.push(`${file}:${i + 1}`);
        }
      });
    }
    if (violations.length > 0) {
      console.warn(`    ⚠  ${violations.length} console.log/warn/error in source (use log() from output-channel)`);
    }
  }));

  // REG-009: data/ must be gitignored.
  all.push(check('REG-009', 'data/ is listed in .gitignore', () => {
    const gi = path.join(ROOT, '.gitignore');
    if (!fs.existsSync(gi)) { return; }
    const content = fs.readFileSync(gi, 'utf8');
    assert(content.includes('data/') || content.includes('/data'),
      'data/ is not in .gitignore — bg-health.json should not be committed.');
  }));

  // REG-010: No dead dependencies.
  all.push(check('REG-010', 'Every package in dependencies is imported somewhere in src/ or mcp-server/src/', () => {
    const pkg  = readJson(path.join(ROOT, 'package.json'));
    const deps = Object.keys(pkg.dependencies ?? {});
    if (deps.length === 0) { return; }
    const extSrc = srcContents()
      .filter(({ file }) => !file.includes('scripts') && !file.includes('tests') && !file.includes('mcp-server'))
      .map(({ src }) => stripTemplateLiterals(src))
      .join('\n');
    let mcpSrc = '';
    const mcpRoot = path.join(ROOT, 'mcp-server', 'src');
    if (fs.existsSync(mcpRoot)) {
      mcpSrc = walkTs(mcpRoot).map(f => stripTemplateLiterals(fs.readFileSync(f, 'utf8'))).join('\n');
    }
    const combined = extSrc + '\n' + mcpSrc;
    const unused = deps.filter(dep => !new RegExp(`from\\s+['"]${dep}|require\\(['"]${dep}`).test(combined));
    assert(unused.length === 0,
      `Packages in "dependencies" but never imported:\n  ${unused.join('\n  ')}\n\nDead deps bloat the VSIX (REG-010).`);
  }));

  // REG-011: Dependency packaging check.
  all.push(check('REG-011', 'Every runtime dependency has a !node_modules/<pkg>/** entry in .vscodeignore', () => {
    const usesEsbuild = fs.existsSync(path.join(ROOT, 'esbuild.mjs'));
    if (usesEsbuild) { return; }
    const pkg    = readJson(path.join(ROOT, 'package.json'));
    const deps   = Object.keys(pkg.dependencies ?? {});
    if (deps.length === 0) { return; }
    const ignore = fs.readFileSync(path.join(ROOT, '.vscodeignore'), 'utf8');
    const missing = deps.filter(d => !ignore.includes(`!node_modules/${d}/`));
    assert(missing.length === 0,
      `Runtime deps missing from .vscodeignore negation:\n  ${missing.join('\n  ')}`);
  }));

  // REG-012: Static HTML files are copied to out/.
  all.push(check('REG-012', 'Static HTML files are copied to out/ by copy-commandhelp.js', () => {
    const missing = ['out/catalog.html'].filter(f => !fs.existsSync(path.join(ROOT, f)));
    assert(missing.length === 0,
      `Missing from out/:\n  ${missing.join('\n  ')}\nRun: node scripts/copy-commandhelp.js`);
  }));

  // REG-013: All run commands route through cvsJobResult output webview.
  all.push(subprocess('REG-013', 'All run commands route through cvsJobResult output webview',
    path.join(ROOT, 'scripts', 'test-run-output-webview.js')));

  // REG-014: Home Recent Runs shows immediate full-string hover tooltip.
  all.push(subprocess('REG-014', 'Home Recent Runs shows immediate full-string hover tooltip',
    path.join(ROOT, 'tests', 'home-recent-runs-tooltip.test.js')));

  // ── Auto-discovered file-based regression tests ───────────────────────────────
  // Every tests/regression/REG-*.test.js runs automatically — no manual wiring needed.

  const regressionDir = path.join(ROOT, 'tests', 'regression');
  const regFiles = fs.readdirSync(regressionDir)
    .filter(f => /^REG-\d+.*\.test\.js$/.test(f))
    .sort();

  for (const file of regFiles) {
    all.push(regressionFile(file));
  }

  // ── Wait for all concurrent tests ────────────────────────────────────────────

  await Promise.all(all);

  console.log('\n' + '─'.repeat(50));
  if (failed === 0) {
    console.log(`✓ All ${passed} regression tests passed — proceeding with build.\n`);
    process.exit(0);
  } else {
    console.error(`\n✗ ${failed} regression test(s) FAILED — build aborted.\n`);
    for (const f of failures) {
      console.error(`  ${f.id}: ${f.name}`);
      console.error(`    ${f.message.split('\n')[0]}`);
    }
    console.error('\nFix the above before rebuilding.\n');
    process.exit(1);
  }
}

main().catch(e => { console.error(e); process.exit(1); });
