...
export function buildCatalogInitPayload(
    ...
    const categorySections = sortedCategories.map(([catLabel, catCards]) => {
        const cardHtml = catCards.map(card => {
            ...
            const folderLabel = card.folder || '(project root)';

            // Build tooltip: Where, When, Why, How + folder
            const tooltipText = [
                ...
                ``,
                `Folder: ${folderLabel}  |  ${card.fileName}`,
            ].join('\n');
  ...
  <div class="card-folder-row">
    <span class="card-folder" data-action="filter-folder" data-project="${esc(card.projectName)}" data-section="${esc(section)}" title="Show only the docs in ${esc(card.projectName)} / ${esc(section)}">&#128193; ${esc(folderLabel)}</span>
  ...
  <h2 class="cat-heading">
    <span class="cat-list" data-action="show-cat-list" style="cursor:pointer" title="Jump to another project">&#9776;</span>
    ${esc(catLabel)}
