...
const BATCH_VIEW_TYPE = 'readmeBatchFix';
...
/** One file in the batch review; the panel itself lives in shared/file-review (#798). */
type BatchItem = ReviewItem;
...
/**
 * Opens the batch review panel (src/shared/file-review.ts): one diff per item,
 * each approved or skipped by the user. Only the files the user approves are
 * written, and only with the content this side proposed: a path the review
 * was not given is never written, whatever the webview message says (#794).
 */
function showBatchReview(items: BatchItem[], title = 'AI Batch Fix Review'): void {
    showFileReview(items, {
        title,
        viewType: BATCH_VIEW_TYPE,
        onApplied: async (result) => {
            for (const p of result.ignored) { log(FEATURE, `Batch review: ignored a path it was not given: ${p}`); }
            for (const r of result.refused) {
                const err = r.error ?? r.detail;
                logError(`Failed to write ${r.filePath}`, err instanceof Error ? err.stack || String(err) : String(err), FEATURE);
            }
...
export function deactivate(): void {
    ...
    disposeFileReview(BATCH_VIEW_TYPE);
    _diffPanel?.dispose();
