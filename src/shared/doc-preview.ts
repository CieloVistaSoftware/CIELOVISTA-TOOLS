...
(function(){
    ...
    function findAnchorTarget(anchorId) {
        ...
        const escaped = (window.CSS && CSS.escape)
            ? CSS.escape(decoded)
            // The page is a String.raw template, so this line is what the
            // page gets. In a plain template the regex lost a backslash, a
            // syntax error that stopped the whole script (#841, #846).
            : decoded.replace(/(["\\])/g, '\\$1');
