...
exports.buildHelpPanelHtml = buildHelpPanelHtml;
/**
 * help-panel.ts
 *
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^```/.test(raw)) {
            if (!inCode) {
                ...
                htmlLines.push(`<pre><code class="lang-${(0, webview_utils_1.esc)(lang)}">`);
            }
            else {
                inCode = false;
        ...
        const h4 = raw.match(/^#### (.+)$/);
        if (h4) {
            htmlLines.push(`<h4>${inlineHtml(h4[1])}</h4>`);
            continue;
        }
        const h3 = raw.match(/^### (.+)$/);
        if (h3) {
            htmlLines.push(`<h3>${inlineHtml(h3[1])}</h3>`);
            continue;
        }
        const h2 = raw.match(/^## (.+)$/);
        if (h2) {
            htmlLines.push(`<h2>${inlineHtml(h2[1])}</h2>`);
            continue;
        }
        const h1 = raw.match(/^# (.+)$/);
        if (h1) {
            htmlLines.push(`<h1>${inlineHtml(h1[1])}</h1>`);
            continue;
        }
        // HR
        if (/^---+$/.test(raw)) {
        ...
        const bq = raw.match(/^> (.+)$/);
        if (bq) {
            htmlLines.push(`<blockquote>${inlineHtml(bq[1])}</blockquote>`);
            continue;
        }
        // Lists
        const li = raw.match(/^[-*] (.+)$/) || raw.match(/^\d+\. (.+)$/);
        if (li) {
            htmlLines.push(`<li>${inlineHtml(li[1])}</li>`);
            continue;
        }
        // Blank line
        if (!raw.trim()) {
...
function inlineHtml(s) {
    ...
    s = s.replace(/\[([^\]]+)\]\(command:(cvs\.[a-zA-Z0-9_.]+)\)/g, (_match, label, cmdId) => `<code>${label}</code> <button class="inline-run-btn" data-action="run" data-id="${(0, webview_utils_1.esc)(cmdId)}" title="Run ${(0, webview_utils_1.esc)(cmdId)}">&#9654; Run</button>`);
    // Bold
...
// ─── HTML builder ─────────────────────────────────────────────────────────────
function buildHelpPanelHtml(markdown, commands, featureName) {
  ...
  <div class="qa-grid">
    ...
    <div class="qa-card">
      <div class="qa-card-top">
        <div class="qa-title">${(0, webview_utils_1.esc)(cmd.title)}</div>
        ${cmd.dewey ? `<span class="qa-dewey">${(0, webview_utils_1.esc)(cmd.dewey)}</span>` : ''}
      </div>
      <div class="qa-desc">${(0, webview_utils_1.esc)(cmd.description)}</div>
      <div class="qa-footer">
        <code class="qa-id">${(0, webview_utils_1.esc)(cmd.id)}</code>
        <button class="qa-run-btn" data-action="run" data-id="${(0, webview_utils_1.esc)(cmd.id)}">&#9654; Run</button>
      </div>
    </div>`).join('')}
  </div>
</div>`;
...
<title>${(0, webview_utils_1.esc)(featureName)} — Help</title>
<style>
${webview_utils_1.CVS_CSS}
...
<div class="help-toolbar">
  ...
  <span class="help-toolbar-title">📄 ${(0, webview_utils_1.esc)(featureName)}</span>
</div>
