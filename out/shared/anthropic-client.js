...
exports.promptForAnthropicKey = promptForAnthropicKey;
...
function getAnthropicKey() {
    return vscode.workspace.getConfiguration('cielovistaTools.anthropic').get('apiKey')?.trim() || undefined;
}
function getOpenAiKey() {
    return vscode.workspace.getConfiguration('cielovistaTools.openai').get('apiKey')?.trim() || undefined;
}
...
async function callOpenAi(prompt, maxTokens, key) {
...
async function callCopilot(prompt, maxTokens) {
...
async function promptForAnthropicKey() {
    ...
    await vscode.workspace.getConfiguration('cielovistaTools.anthropic').update('apiKey', key.trim(), vscode.ConfigurationTarget.Global);
    return key.trim();
}
// ─── Public API ───────────────────────────────────────────────────────────────
/**
...
async function callClaude(prompt, maxTokens = 2000) {
    ...
    const action = await vscode.window.showErrorMessage('All AI providers failed. Set an API key in settings.', 'Set Anthropic Key', 'Set OpenAI Key', 'Open Settings');
    ...
    if (action === 'Set OpenAI Key') {
        ...
        if (key?.trim()) {
            await vscode.workspace.getConfiguration('cielovistaTools.openai').update('apiKey', key.trim(), vscode.ConfigurationTarget.Global);
            return callClaude(prompt, maxTokens);
        }
    }
