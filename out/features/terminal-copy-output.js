...
const FEATURE = 'terminal-copy-output';
 ...
 */
async function sendToCopilotChat(content) {
 ...
 */
async function copyTerminalOutput(opts = {}) {
    ...
    const originalClip = await vscode.env.clipboard.readText();
    let copied = false;
    ...
    try {
        ...
        if (text && text !== originalClip) {
            copied = true;
        }
    }
    catch { /* shell integration not available */ }
    if (!copied) {
        vscode.window.showWarningMessage('Could not capture last command output. Try selecting the text manually, then run this command again.');
        return false;
    }
    ...
    (0, output_channel_1.log)(FEATURE, `Copied ${lines} lines from terminal`);
    if (opts.pasteToChat) {
        try {
            const pasted = await sendToCopilotChat(content);
            if (!pasted) {
            ...
            (0, output_channel_1.logError)(FEATURE, 'Failed to paste to Copilot chat', err);
            vscode.window.showWarningMessage('Could not send to chat — content is on your clipboard. Press Ctrl+V in the chat input.');
        }
    }
    return true;
}
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cvs.terminal.copyOutputClipboard', () => copyTerminalOutput({ formatAsMarkdown: true })), vscode.commands.registerCommand('cvs.terminal.pasteOutputToChat', () => copyTerminalOutput({ pasteToChat: true, formatAsMarkdown: true })));
}
function deactivate() { }
// ─── Utility ─────────────────────────────────────────────────────────────────
function delay(ms) {
...
//# sourceMappingURL=terminal-copy-output.js.map