...
const consolidation_plan_webview_1 = require("../shared/consolidation-plan-webview");
...
// ─── Doc scanner ──────────────────────────────────────────────────────────────
function scanDir(rootPath, projectName, projectRootPath, maxDepth = 3) {
    const results = [];
...
// ─── Discovery ────────────────────────────────────────────────────────────────
function discoverGroups(allDocs) {
    const groups = [];
    ...
    for (let i = 0; i < allDocs.length; i++) {
        for (let j = i + 1; j < allDocs.length; j++) {
            ...
            if (a.sizeBytes < 150 || b.sizeBytes < 150) {
                continue;
            }
...
function appendToLog(entry) {
    ...
    (0, output_channel_1.log)(FEATURE, `Log updated: ${CONSOLIDATION_LOG}`);
}
// ─── Reference updater ────────────────────────────────────────────────────────
/**
 ...
 */
function updateReferences(oldPath, newPath, projects) {
    const updated = [];
    for (const project of projects) {
        ...
        try {
            ...
            if (updated_content !== content) {
                ...
                (0, output_channel_1.log)(FEATURE, `Updated references in: ${claudeMd}`);
            }
        }
        catch (err) {
            (0, output_channel_1.logError)(FEATURE, `Failed to update references in ${claudeMd}`, err);
        }
    }
    return updated;
}
 ...
 */
async function consolidateGroup(group, registry) {
    // Step 1 — pick the keeper
    const keeperPick = await vscode.window.showQuickPick(group.files.map(f => ({
    ...
    let result = await new Promise(resolve => {
        (0, consolidation_plan_webview_1.showConsolidationPlanWebview)(plan, async (checkedPlan) => {
            ...
            for (const action of checkedPlan) {
                try {
                    ...
                    (0, output_channel_1.logError)(FEATURE, `Failed to ${action.type} ${action.filePath}`, err);
                    ok = false;
                }
            }
            resolve(ok);
 ...
 */
function buildConsolidationPlan(group, keeper, destPath, registry) {
    const actions = [];
    const isMoving = destPath !== keeper.filePath;
...
async function runConsolidation() {
    ...
    const allDocs = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Scanning all projects for duplicate docs…' }, async (progress) => {
        const docs = scanDir(registry.globalDocsPath, 'global', registry.globalDocsPath);
        for (const project of registry.projects) {
    ...
    if (!groups.length) {
        require('../shared/show-result-webview').showResultWebview('No Duplicates Found', 'Run Doc Consolidation', 0, `✅ No duplicate docs found across <b>${allDocs.length}</b> files scanned. Everything is already one-time-one-place.`);
        return;
    }
    require('../shared/show-result-webview').showResultWebview('Consolidation Opportunities', 'Run Doc Consolidation', 0, `Found <b>${groups.length}</b> consolidation opportunities across <b>${allDocs.length}</b> docs.`);
    ...
    require('../shared/show-result-webview').showResultWebview('Consolidation Complete', 'Run Doc Consolidation', 0, `Consolidation complete. <b>${consolidated}</b> groups consolidated, <b>${skipped}</b> skipped. <a href="#" onclick="window.acquireVsCodeApi().postMessage({openLog:true})">Open Log</a>`);
    // Optionally, open log if user clicks in webview (handled by message)
}
/** Consolidate only docs with the same filename. */
async function consolidateByName() {
    const registry = (0, registry_1.loadRegistry)();
    if (!registry) {
        return;
    }
    const allDocs = scanDir(registry.globalDocsPath, 'global', registry.globalDocsPath);
    for (const p of registry.projects) {
        if (fs.existsSync(p.path)) {
            allDocs.push(...scanDir(p.path, p.name, p.path));
        }
    }
...
async function consolidateByContent() {
    const registry = (0, registry_1.loadRegistry)();
    if (!registry) {
        return;
    }
    ...
    for (const item of selected) {
        ...
        const showDiff = await vscode.window.showInformationMessage(`Review diff for "${item.group.label}" before consolidating?`, 'Show Diff', 'Skip Diff', 'Skip This');
        if (showDiff === 'Skip This') {
            continue;
        }
        if (showDiff === 'Show Diff') {
            await vscode.commands.executeCommand('vscode.diff', vscode.Uri.file(a.filePath), vscode.Uri.file(b.filePath), `${a.fileName} ↔ ${b.fileName}`);
            const proceed = await vscode.window.showInformationMessage('Consolidate these two docs?', 'Yes', 'No');
            if (proceed !== 'Yes') {
                continue;
            }
        }
...
async function openConsolidationLog() {
    if (!fs.existsSync(CONSOLIDATION_LOG)) {
        require('../shared/show-result-webview').showResultWebview('No Consolidation Log', 'Open Consolidation Log', 0, 'No consolidation log yet. Run a consolidation first.');
        return;
    }
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cvs.consolidate.run', runConsolidation), vscode.commands.registerCommand('cvs.consolidate.byName', consolidateByName), vscode.commands.registerCommand('cvs.consolidate.byContent', consolidateByContent), vscode.commands.registerCommand('cvs.consolidate.log', openConsolidationLog));
}
function deactivate() { }
//# sourceMappingURL=doc-consolidator.js.map