...
exports.runCodebaseAudit = runCodebaseAudit;
/**
 * codebase-auditor.ts
 *
...
// ─── Checks ───────────────────────────────────────────────────────────────────
let _seq = 0;
function id(cat) { return `${cat}-${++_seq}`; }
/** 1. File size */
...
function checkFunctionLength(files) {
    const findings = [];
...
function checkDuplicateExports(files) {
    const findings = [];
    const exportMap = new Map(); // name -> [file, file, ...]
    ...
    for (const [name, fileList] of exportMap) {
        ...
        if (['activate', 'deactivate'].includes(name)) {
            continue;
        }
...
function checkDeadMonoliths(files) {
    const findings = [];
...
function checkMissingReadmes(files) {
    const findings = [];
...
function checkOneTimeOnePlace(files) {
    const findings = [];
...
function checkSharedUtilUsage(files) {
    const findings = [];
    const featureFiles = files.filter(f => f.rel.startsWith('features/'));
    for (const f of featureFiles) {
...
function checkDeadFiles(files) {
    ...
    const featureTopLevel = files.filter(f => f.rel.startsWith('features/') &&
        !f.rel.includes('/') &&
        f.lines > 20);
...
function buildAuditHtml(findings, scannedFiles, totalLines) {
...
<div class="filter-bar">
  ...
  ${categories.map(c => `<button class="filter-btn" data-filter="cat:${esc(c)}">${esc(c)}</button>`).join('')}
</div>
...
// ─── Command ─────────────────────────────────────────────────────────────────
async function runCodebaseAudit() {
    let { findings, files } = runScan();
    const totalLines = files.reduce((n, f) => n + f.lines, 0);
    const panel = vscode.window.createWebviewPanel('codebaseAudit', '🔬 Codebase Audit', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
    panel.webview.html = buildAuditHtml(findings, files.length, totalLines);
    (0, output_channel_1.log)(FEATURE, `Codebase audit: ${files.length} files, ${findings.length} findings`);
    panel.webview.onDidReceiveMessage(async (msg) => {
        try {
            ...
            (0, output_channel_1.logError)(FEATURE, 'Codebase audit action failed', err);
            panel.webview.postMessage({ type: 'error', text: String(err) });
        }
    });
}
//# sourceMappingURL=codebase-auditor.js.map