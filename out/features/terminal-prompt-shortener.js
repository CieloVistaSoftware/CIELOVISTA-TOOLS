...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cvs.terminal.togglePromptLength', () => {
        _isShort = !_isShort;
        const terminal = (0, terminal_utils_1.getActiveOrCreateTerminal)();
        terminal.show();
        terminal.sendText(_isShort ? SHORT_PROMPT : FULL_PROMPT);
        vscode.window.showInformationMessage(`Terminal prompt: ${_isShort ? 'SHORT (>)' : 'FULL (path)'}`);
        (0, output_channel_1.log)(FEATURE, `Prompt toggled to ${_isShort ? 'short' : 'full'}`);
    }));
}
function deactivate() { }
//# sourceMappingURL=terminal-prompt-shortener.js.map