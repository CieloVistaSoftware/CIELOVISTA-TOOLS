...
function guessLanguage(firstLine) {
...
function buildHtml(blocks, scannedFiles) {
    const byProject = new Map();
    for (const b of blocks) {
        if (!byProject.has(b.project)) {
            byProject.set(b.project, []);
        }
    ...
    const rows = [...byProject.entries()].map(([proj, pBlocks]) => {
        ...
        for (const b of pBlocks) {
            if (!fileMap.has(b.filePath)) {
                fileMap.set(b.filePath, []);
            }
...
async function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('cvs.audit.codeHighlight', showPanel));
}
async function showPanel() {
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Scanning docs for untagged code blocks…', cancellable: false }, async () => {
        ...
        if (_panel) {
            ...
            _panel = vscode.window.createWebviewPanel('codeHighlightAudit', '✨ Code Highlight Audit', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
            _panel.webview.html = html;
        ...
        _panel.webview.onDidReceiveMessage(async (msg) => {
            if (msg.command === 'open' && msg.path) {
                try {
                    ...
                    (0, output_channel_1.logError)(FEATURE, 'Failed to open file', e);
                }
            }
        ...
        (0, output_channel_1.log)(FEATURE, `Code highlight audit: ${blocks.length} untagged blocks in ${scannedFiles} files`);
    });
}
function deactivate() {
    _panel?.dispose();
...
//# sourceMappingURL=code-highlight-audit.js.map