...
exports.buildLauncherHtml = buildLauncherHtml;
const catalog_1 = require("./catalog");
function statusDotHtml(status, checkId, summary) {
    ...
    return `<button class="card-status-dot card-status-dot--clickable" style="background:${color}" data-action="show-audit" data-check-id="${(0, catalog_1.esc)(checkId ?? '')}" data-status="${(0, catalog_1.esc)(status)}" data-summary="${(0, catalog_1.esc)(safeSummary)}" title="${status === 'red' ? '🔴 Issues found' : '🟡 Warnings'} — click for details &amp; fixes"></button>`;
}
const SCOPE_META = {
...
function buildCard(cmd, auditMap, group) {
    const tagBadges = cmd.tags.map(t => `<span class="tag" data-tag="${(0, catalog_1.esc)(t)}">${(0, catalog_1.esc)(t)}</span>`).join('');
    ...
    const scopeBadge = `<span class="scope-badge" style="border-color:${scopeMeta.color};color:${scopeMeta.color}" title="${(0, catalog_1.esc)(scopeMeta.title)}">${(0, catalog_1.esc)(scopeMeta.label)}</span>`;
    const breadcrumb = `<div class="cmd-breadcrumb"><button class="bc-root bc-btn" data-action="bc-root" title="Go to root — show all commands">CieloVista Tools</button><span class="bc-s">\\</span><button class="bc-group bc-btn" data-action="bc-group" data-group="${(0, catalog_1.esc)(group)}" title="Filter to ${(0, catalog_1.esc)(group)} only">${(0, catalog_1.esc)(group)}</button><span class="bc-s">\\</span><span class="bc-leaf">${(0, catalog_1.esc)(cmd.title)}</span></div>`;
    // F1 data embedded for the help modal
    const f1Data = (0, catalog_1.esc)(JSON.stringify({
        id: cmd.id,
        title: cmd.title,
    ...
    return `<div class="cmd-card" data-id="${(0, catalog_1.esc)(cmd.id)}" data-group="${(0, catalog_1.esc)(group)}" data-tags="${(0, catalog_1.esc)(cmd.tags.join(' '))}" data-scope="${(0, catalog_1.esc)(cmd.scope)}" tabindex="0" role="article" aria-label="${(0, catalog_1.esc)(cmd.title)}">
  ${dot}
  ${breadcrumb}
  <div class="cmd-header">
    <h1 class="cmd-title">${(0, catalog_1.esc)(cmd.title)}</h1>
    <span class="cmd-dewey" title="Dewey catalogue number: ${(0, catalog_1.esc)(cmd.dewey)}">${(0, catalog_1.esc)(cmd.dewey)}</span>
  </div>
  <p class="cmd-desc">${(0, catalog_1.esc)(liveSummary)}</p>
  <div class="cmd-footer">
    ...
    <div class="cmd-actions">
      ...
      ${cmd.helpDoc ? `<button class="help-btn" data-action="help" data-doc="${(0, catalog_1.esc)(cmd.helpDoc)}" title="Open detailed help document for ${(0, catalog_1.esc)(cmd.title)}">Help</button>` : ''}
      <button class="run-btn ${cmd.action === 'read' ? 'read-btn' : ''}" data-action="run" data-id="${(0, catalog_1.esc)(cmd.id)}" title="${cmd.action === 'read' ? 'Read: open' : 'Run:'} ${(0, catalog_1.esc)(cmd.title)}">${cmd.action === 'read' ? ICON_READ : ICON_PLAY} ${cmd.action === 'read' ? 'Open' : 'Run'}</button>
    </div>
  </div>
</div>`;
}
function buildLocationBar(wsPath) {
    if (!wsPath) {
        return '';
    }
    ...
    const crumbs = parts.map((seg, i) => {
        ...
        const dispSep = sep;
        const sepHtml = i > 0 ? `<span class="loc-sep">${(0, catalog_1.esc)(dispSep)}</span>` : '';
        if (isLast) {
            return `${sepHtml}<span class="loc-crumb loc-current">${(0, catalog_1.esc)(seg)}</span>`;
        }
        return `${sepHtml}<button class="loc-crumb loc-link" data-path="${(0, catalog_1.esc)(fullPath)}" title="Open ${(0, catalog_1.esc)(fullPath)} in terminal">${(0, catalog_1.esc)(seg)}</button>`;
    }).join('');
...
function buildLauncherHtml(report, workspacePath) {
    const auditMap = new Map();
    if (report) {
        for (const c of report.checks) {
            auditMap.set(c.checkId, { status: c.status, summary: c.summary });
        }
    }
  ...
  <button data-action="run" data-id="cvs.audit.runDaily">🔍 Run Initial Audit</button>
    ...
    for (const cmd of catalog_1.CATALOG) {
        if (!byGroup.has(cmd.group)) {
            byGroup.set(cmd.group, []);
        }
        byGroup.get(cmd.group).push(cmd);
    }
    let groupSections = [...byGroup.entries()].map(([group, cmds]) => `<section class="group-section" data-group="${(0, catalog_1.esc)(group)}">
  <h2 class="group-heading">${(0, catalog_1.esc)(group)} <span class="group-count">${cmds.length}</span></h2>
  <div class="cmd-grid">${cmds.map(cmd => buildCard(cmd, auditMap, group)).join('')}</div>
</section>`).join('');
    const testCmds = catalog_1.CATALOG.filter(cmd => cmd.tags.includes('test'));
  ...
  <div class="cmd-grid">${testCmds.map(cmd => buildCard(cmd, auditMap, 'Tests')).join('')}</div>
    ...
    const groupBtns = (0, catalog_1.allGroups)().map(g => {
        let icon = catalog_1.CATALOG.find(c => c.group === g)?.groupIcon ?? '';
        if (g === 'Tests') {
            icon = '🧪';
        }
        return `<button class="group-btn" data-group="${(0, catalog_1.esc)(g)}" title="Show ${(0, catalog_1.esc)(g)} commands only">${(0, catalog_1.esc)(g)}</button>`;
    }).join('');
    const topicCheckboxes = (0, catalog_1.allTags)().map(t => `<label class="dd-item"><input type="checkbox" class="topic-cb" value="${(0, catalog_1.esc)(t)}"><span>${(0, catalog_1.esc)(t)}</span></label>`).join('');
    const locationBarHtml = buildLocationBar(workspacePath);
    const wsPathJs = (workspacePath ?? '').replace(/\\/g, '\\\\');
    const catalogJson = JSON.stringify(catalog_1.CATALOG);
    const total = catalog_1.CATALOG.length;
    const CSS = `
...
<div id="toolbar">
  <div id="toolbar-top">
    ...
    ${auditAgeStr ? `<span style="font-size:10px;color:var(--vscode-descriptionForeground)">&#x23F1; ${(0, catalog_1.esc)(auditAgeStr)}</span>` : ''}
  </div>
  <div id="group-bar">
