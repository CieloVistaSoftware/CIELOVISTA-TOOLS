...
exports.buildComplianceHtml = buildComplianceHtml;
exports.buildSummaryHtml = buildSummaryHtml;
...
function buildComplianceHtml(results) {
        ...
        const issueHtml = r.issues.length === 0
            ...
            : r.issues.map(i => {
                ...
                const fixBtn = i.fixable
                    ? `<button class="cvs-btn-sm" data-action="fix-one" data-proj="${(0, webview_utils_1.esc)(r.project.name)}">Fix</button>`
                    : '';
                return `<div class="cvs-issue">${icon} <span class="cvs-issue-file">${(0, webview_utils_1.esc)(i.file)}</span> — ${(0, webview_utils_1.esc)(i.message)} ${fixBtn}</div>`;
            }).join('');
        return `<tr>
  <td><button class="cvs-link-btn" data-action="open-folder" data-path="${(0, webview_utils_1.esc)(r.project.path)}">${(0, webview_utils_1.esc)(r.project.name)}</button></td>
  <td><span class="cvs-badge">${(0, webview_utils_1.esc)(r.project.type)}</span></td>
  <td><span style="font-weight:700;color:${scoreColor(r.score)}">${r.score}</span></td>
  <td style="max-width:360px">${issueHtml}</td>
  <td style="white-space:nowrap">
    ${r.issues.some(i => i.fixable) ? `<button class="cvs-btn" data-action="fix-one" data-proj="${(0, webview_utils_1.esc)(r.project.name)}">🔧 Fix</button>` : ''}
    <button class="cvs-btn-secondary" data-action="open-folder" data-path="${(0, webview_utils_1.esc)(r.project.path)}">📂 Open</button>
  </td>
</tr>`;
    }).join('');
...
function buildSummaryHtml(lines, totalFiles, totalProjects) {
    const ICONS = { 'LICENSE': '📄', 'CHANGELOG.md': '📝', 'icon.png': '🎨', 'package.json': '📦' };
    const rows = lines.map(line => {
        const [proj, ...rest] = line.split(': ');
        const files = rest.join(': ').split(', ');
        const fileHtml = files.map(f => `<span style="display:inline-flex;align-items:center;gap:4px;background:var(--vscode-textCodeBlock-background);border:1px solid var(--vscode-panel-border);border-radius:3px;padding:2px 8px;font-size:11px;margin:2px 3px 2px 0">${ICONS[f.trim()] ?? '✅'} ${(0, webview_utils_1.esc)(f.trim())}</span>`).join('');
        return `<tr><td style="font-weight:700;white-space:nowrap;padding:6px 10px">${(0, webview_utils_1.esc)(proj)}</td><td style="padding:6px 10px">${fileHtml}</td></tr>`;
    }).join('');
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">
