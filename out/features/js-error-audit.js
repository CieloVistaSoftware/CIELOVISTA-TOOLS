...
// ─── Project root ─────────────────────────────────────────────────────────────
function findDiskCleanUpRoot() {
    const registry = (0, registry_1.loadRegistry)();
    if (registry) {
        const entry = registry.projects.find(p => p.name.toLowerCase().includes('diskcleanup') ||
...
function findAuditJson(root) { return path.join(root, 'data', 'js-error-audit.json'); }
function findStateJson(root) { return path.join(root, 'data', 'js-error-audit-state.json'); }
function findAuditScript(root) { return path.join(root, 'scripts', 'js-error-audit.js'); }
// ─── State management ─────────────────────────────────────────────────────────
...
function mergeState(root, report) {
    ...
    const allFindings = [
        ...report.violations.map(v => ({ v, severity: 'ERROR' })),
        ...report.warnings.map(v => ({ v, severity: 'WARN' })),
    ];
    const nextEntries = [];
    ...
    const newState = { entries: nextEntries, lastSeen: new Date().toISOString() };
    saveState(root, newState);
    return newState;
}
function updateEntryStatus(root, id, status) {
    const state = loadState(root);
...
function classifyFixKind(v) {
    const issues = v.issues ?? (v.issue ? [v.issue] : []);
    if (issues.some(i => i.includes('bare catch'))) {
        return 'BARE_CATCH';
    }
    if (issues.some(i => i.includes('ErrLog'))) {
        return 'NO_ERRLOG';
    }
    return 'NO_TRY_CATCH'; // zero try/catch — needs wrapping
}
// ─── Line finder for warnings ─────────────────────────────────────────────────
function findIssueLine(absPath, fixKind) {
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (fixKind === 'BARE_CATCH' && /catch\s*(?:\([^)]*\))?\s*\{\s*\}/.test(lines[i])) {
                return i + 1;
            }
            if (fixKind === 'NO_ERRLOG' && /\bcatch\s*[({]/.test(lines[i])) {
                return i + 1;
            }
        }
    }
...
function collapseDiff(diff, context = 3) {
    const changed = new Set();
    diff.forEach((d, i) => { if (d.kind !== 'eq') {
        changed.add(i);
    } });
    const keep = new Set();
...
// ─── AI fix generation ────────────────────────────────────────────────────────
async function generateAiFix(absPath, entry) {
...
6. Return ONLY the complete corrected JavaScript file. No explanation, no markdown fences.`;
    ...
    const result = await (0, anthropic_client_1.callClaude)(prompt, 8000);
...
function buildAuditHtml(report, state, root) {
...
// ─── Core commands ────────────────────────────────────────────────────────────
async function runJsErrorAudit() {
    try {
        const root = findDiskCleanUpRoot();
        if (!root) {
            (0, output_channel_1.logError)(FEATURE, 'Could not find DiskCleanUp project. Check your project registry.');
            vscode.window.showErrorMessage('Could not find DiskCleanUp project. Check your project registry.');
            return;
        }
        let report = loadReport(root);
        if (!report) {
            const run = await vscode.window.showWarningMessage('No js-error-audit.json found. Run the audit first?', 'Run npm run audit:errors', 'Cancel');
            if (run !== 'Run npm run audit:errors') {
                return;
            }
            await runAuditScript(root);
            report = loadReport(root);
            if (!report) {
                (0, output_channel_1.logError)(FEATURE, 'Audit failed — check the terminal.');
        ...
        (0, output_channel_1.logError)(FEATURE, 'runJsErrorAudit failed', err);
...
function loadReport(root) {
    ...
    try {
        ...
        (0, output_channel_1.logError)(FEATURE, 'Failed to parse audit JSON', err);
        return undefined;
    }
}
...
// ─── Panel setup ──────────────────────────────────────────────────────────────
function showPanel(report, state, root) {
    ...
    if (_panel) {
        ...
        _panel = vscode.window.createWebviewPanel('jsErrorAudit', '🛡️ JS Error Audit', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
        _panel.webview.html = html;
        _panel.onDidDispose(() => { _panel = undefined; _pendingFix.clear(); _fixQueue.length = 0; });
    }
    attachHandler(root);
    (0, output_channel_1.log)(FEATURE, `Panel opened — ${report.violations.length} violations, ${report.warnings.length} warnings`);
}
// ─── Message handler ──────────────────────────────────────────────────────────
function attachHandler(root) {
    ...
    _panel.webview.onDidReceiveMessage(async (msg) => {
        try {
            switch (msg.command) {
                case 'open': {
                    try {
                        if (!fs.existsSync(msg.path)) {
                            (0, output_channel_1.logError)(FEATURE, `File not found: ${msg.path}`);
                        ...
                        (0, output_channel_1.logError)(FEATURE, `Failed to open file: ${msg.path}`, err);
                ...
                case 'aiFix': {
                    try {
                        ...
                        (0, output_channel_1.logError)(FEATURE, `AI fix failed for ${msg.id}`, err);
                ...
                case 'fixAll': {
                    try {
                        ...
                        (0, output_channel_1.logError)(FEATURE, 'Fix All failed', err);
                ...
                case 'accept': {
                    try {
                        ...
                        if (!pending) {
                            (0, output_channel_1.logError)(FEATURE, `No pending fix found for ${msg.id}`);
                        ...
                        (0, output_channel_1.log)(FEATURE, `Accepted fix for ${msg.id} — file written`);
                        ...
                        (0, output_channel_1.logError)(FEATURE, `Accept fix failed for ${msg.id}`, err);
                ...
                case 'reject': {
                    try {
                        ...
                        (0, output_channel_1.logError)(FEATURE, `Reject fix failed for ${msg.id}`, err);
                ...
                case 'exclude': {
                    try {
                        ...
                        if (!fs.existsSync(scriptPath)) {
                            (0, output_channel_1.logError)(FEATURE, 'Audit script not found.');
                        ...
                        if (!scriptSrc.includes(`'${basename}'`)) {
                            scriptSrc = scriptSrc.replace(/const INTENTIONAL_NO_CATCH = new Set\(\[/, `const INTENTIONAL_NO_CATCH = new Set([\n  '${basename}',   // pure module — excluded via JS Error Audit`);
                            fs.writeFileSync(scriptPath, scriptSrc, 'utf8');
                        }
                        updateEntryStatus(root, msg.id, 'excluded');
                        (0, output_channel_1.log)(FEATURE, `Excluded ${basename} from audit rule`);
                        await runAuditScript(root);
                        refresh(root);
                    }
                    catch (err) {
                        (0, output_channel_1.logError)(FEATURE, `Exclude failed for ${msg.id}`, err);
                ...
                case 'rerun': {
                    try {
                        ...
                        (0, output_channel_1.logError)(FEATURE, 'Re-run audit failed', err);
                ...
                case 'reload': {
                    try {
                        ...
                        (0, output_channel_1.logError)(FEATURE, 'Reload failed', err);
            ...
            (0, output_channel_1.logError)(FEATURE, `Handler error: ${msg.command}`, err);
            _panel?.webview.postMessage({ type: 'error', text: String(err) });
        }
    });
}
// ─── AI fix helper ────────────────────────────────────────────────────────────
async function handleAiFix(root, id, absPath) {
    const state = loadState(root);
    const entry = state.entries.find(e => e.id === id);
    if (!entry) {
    ...
    try {
        ...
        (0, output_channel_1.logError)(FEATURE, `AI fix failed for ${id}`, err);
        _panel?.webview.postMessage({ type: 'error', text: `AI fix failed for ${id}: ${err}` });
    }
}
// ─── Audit script runner ──────────────────────────────────────────────────────
async function runAuditScript(root) {
    return vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Running JS error audit…', cancellable: false }, async () => {
        try {
            (0, child_process_1.execSync)('npm run audit:errors', { cwd: root, stdio: 'pipe' });
        }
        catch (err) {
            (0, output_channel_1.log)(FEATURE, `Audit script output: ${err?.stdout?.toString() ?? ''}`);
        }
    });
}
// ─── Activate / Deactivate ────────────────────────────────────────────────────
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cvs.audit.jsErrors', runJsErrorAudit));
}
function deactivate() {
    _panel?.dispose();
    _panel = undefined;
...
//# sourceMappingURL=js-error-audit.js.map