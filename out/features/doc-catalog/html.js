...
exports.buildCatalogHtml = buildCatalogHtml;
...
function buildCatalogHtml(cards, projectInfos = [], builtAt = '') {
    ...
    const categorySections = sortedCategories.map(([catLabel, catCards]) => {
        ...
        const seqWithinCat = new Map();
        const cardHtml = catCards.map(card => {
            const relPath = path.relative(card.projectPath, card.filePath).replace(/\\/g, '/');
            const tagsHtml = card.tags.slice(0, 6).map(t => `<span class="tag">${(0, content_1.esc)(t)}</span>`).join('');
            ...
            return `<article class="card" data-id="${card.id}" data-project="${(0, content_1.esc)(card.projectName)}" data-category="${(0, content_1.esc)(card.category)}" data-tags="${(0, content_1.esc)(card.tags.join(' '))}">
  <div class="card-header">
    <span class="card-project">${(0, content_1.esc)(card.projectName)}</span>
    <span class="card-date">${card.lastModified}</span>
  </div>
  <div class="card-dewey-row">
    <span class="card-dewey">${deweyNum}</span>
    <span class="card-filename">${(0, content_1.esc)(card.fileName)}</span>
  </div>
  <div class="card-title" style="cursor:pointer" data-action="open-preview" data-path="${(0, content_1.esc)(card.filePath)}">${(0, content_1.esc)(card.title)}</div>
  <div class="card-desc">${(0, content_1.esc)(card.description)}</div>
  <div class="card-path" title="${(0, content_1.esc)(card.filePath)}">${(0, content_1.esc)(relPath)}</div>
  <div class="card-tags">${tagsHtml}</div>
  <div class="card-footer">
    <span class="card-size">${(card.sizeBytes / 1024).toFixed(1)} KB</span>
    <div class="card-btns">
      <button class="btn-view" data-action="open-preview" data-path="${(0, content_1.esc)(card.filePath)}">📄 View</button>
      <button class="btn-open" data-action="open" data-path="${(0, content_1.esc)(card.filePath)}">↗ Open</button>
      <button class="btn-open" data-action="open-folder" data-proj-path="${(0, content_1.esc)(card.projectPath)}">📂 Folder</button>
    </div>
  </div>
</article>`;
        }).join('');
        ...
        return `<section class="cat-section" data-category="${(0, content_1.esc)(catLabel)}">
  <h2 class="cat-heading"><span class="cat-dewey">${(0, content_1.esc)(baseLabel)}</span> ${(0, content_1.esc)(catLabel)} <span class="cat-count">${catCards.length}</span></h2>
  <div class="card-grid">${cardHtml}</div>
    ...
    const projectOptions = allProjects.map(p => `<option value="${(0, content_1.esc)(p)}">${(0, content_1.esc)(p)}</option>`).join('');
    const categoryOptions = ['🗂️ Projects', ...categoryLabels].map(c => `<option value="${(0, content_1.esc)(c)}">${(0, content_1.esc)(c)}</option>`).join('');
    const projectsSectionHtml = (0, projects_1.buildProjectsSectionHtml)(projectInfos);
...
<div id="toolbar">
  ...
  ${builtAt ? `<span style="font-size:10px;color:var(--vscode-descriptionForeground);white-space:nowrap">⏱ Built ${(0, content_1.esc)(builtAt)}</span>` : ''}
</div>
