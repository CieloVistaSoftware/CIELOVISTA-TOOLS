...
exports.buildProjectsSectionHtml = buildProjectsSectionHtml;
...
function loadProjectInfo(entry) {
    ...
    if (fs.existsSync(readmePath)) {
        try {
            ...
            const fromReadme = (0, content_2.extractDescription)(content);
...
function detectTestStatus(rootPath, hasNpm) {
    const testsDir = path.join(rootPath, 'tests');
    if (!fs.existsSync(testsDir)) {
        return { hasTests: false, hasReal: false, specCount: 0 };
    }
    let specCount = 0;
    let realCount = 0;
    try {
        ...
        for (const f of specFiles) {
            try {
                ...
                if (!isPlaceholder && content.match(/\btest\s*\(/)) {
                    realCount++;
                }
            }
            catch { /* skip */ }
...
function buildProjectsSectionHtml(projects) {
    const cards = projects.map(p => {
        ...
        const scriptBtns = scriptKeys.map(s => {
            ...
            if (s === 'test') {
                if (!testStatus.hasTests || !testStatus.hasReal) {
                    ...
                    return `<button class="btn-open btn-no-tests" data-action="create-tests" data-proj-path="${(0, content_1.esc)(p.rootPath)}" data-proj-name="${(0, content_1.esc)(p.name)}" title="No real tests found — click to generate tests using unit test rules">🧪 Create Tests</button>`;
                }
                // Real tests exist — show normal test button with count badge
                const badge = testStatus.specCount > 0 ? ` (${testStatus.specCount})` : '';
                return `<button class="btn-open" data-action="run" data-proj-path="${(0, content_1.esc)(p.rootPath)}" data-script="test">🧪 test${badge}</button>`;
            }
            return `<button class="${isPrimary ? 'btn-view' : 'btn-open'}" data-action="run" data-proj-path="${(0, content_1.esc)(p.rootPath)}" data-script="${(0, content_1.esc)(s)}">${(0, content_1.esc)(s)}</button>`;
        }).join('');
        ...
        const noTestsBtn = !hasTestScript && p.hasNpm
            ? `<button class="btn-open btn-no-tests" data-action="create-tests" data-proj-path="${(0, content_1.esc)(p.rootPath)}" data-proj-name="${(0, content_1.esc)(p.name)}" title="No test script found — click to generate tests">🧪 Create Tests</button>`
            : '';
        const dotnetBtn = p.hasDotnet && !p.hasNpm
            ? `<button class="btn-view" data-action="run" data-proj-path="${(0, content_1.esc)(p.rootPath)}" data-script="dotnet:build">build</button>` : '';
        const claudePath = path.join(p.rootPath, 'CLAUDE.md');
        const claudeExists = fs.existsSync(claudePath);
        const claudeBtn = claudeExists
            ? `<button class="btn-open" data-action="open-claude" data-proj-path="${(0, content_1.esc)(p.rootPath)}">📄 CLAUDE.md</button>`
            : `<button class="btn-open btn-create-claude" data-action="create-claude" data-proj-path="${(0, content_1.esc)(p.rootPath)}" title="No CLAUDE.md found — click to create one">➕ Create CLAUDE.md</button>`;
        return `<article class="card proj-card" data-project="${(0, content_1.esc)(p.name)}" data-category="🗂️ Projects" data-tags="${(0, content_1.esc)(p.type)}">
  <div class="card-title" style="font-size:1.05em;font-weight:800;margin-bottom:2px">${(0, content_1.esc)(p.name)}</div>
  <div class="card-header"><span class="card-project">${icon} ${(0, content_1.esc)(p.type.toUpperCase())}</span></div>
  <div class="card-desc">${(0, content_1.esc)(p.description)}</div>
  <div class="card-path" title="${(0, content_1.esc)(p.rootPath)}">${(0, content_1.esc)(p.rootPath)}</div>
  <div style="display:flex;flex-direction:column;gap:6px;margin-top:8px">
    <div class="card-btns" style="flex-wrap:wrap;gap:4px">${scriptBtns}${noTestsBtn}${dotnetBtn}</div>
    <div class="card-btns">
      <button class="btn-open" data-action="open-folder" data-proj-path="${(0, content_1.esc)(p.rootPath)}">📂 Open Folder</button>
      ${claudeBtn}
    </div>
  </div>
</article>`;
