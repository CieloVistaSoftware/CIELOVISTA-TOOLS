...
function saveRegistry(registry) {
    fs.writeFileSync(registry_1.REGISTRY_PATH, JSON.stringify(registry, null, 2), 'utf8');
    (0, output_channel_1.log)(FEATURE, 'Project registry saved');
}
// ─── Doc file discovery ───────────────────────────────────────────────────────
...
function listProjectDocs(projectPath) {
    const results = [];
    const candidates = [
        projectPath,
...
async function openDoc(filePath) {
    if (filePath.toLowerCase().endsWith('.md')) {
        (0, doc_preview_1.openDocPreview)(filePath);
        (0, output_channel_1.log)(FEATURE, `Opened (preview): ${filePath}`);
    }
    else {
        const doc = await vscode.workspace.openTextDocument(filePath);
        await vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside);
        (0, output_channel_1.log)(FEATURE, `Opened: ${filePath}`);
    }
}
// --- Extension activation: sync left panel highlight with active editor ---
const commands_1 = require("./doc-catalog/commands");
// ─── Commands ─────────────────────────────────────────────────────────────────
/** Open any file in the global CieloVistaStandards folder — uses the View a Doc webview. */
async function openGlobalDoc() {
...
async function newGlobalDoc() {
    ...
    if (fs.existsSync(filePath)) {
        const overwrite = await vscode.window.showWarningMessage(`${fileName} already exists. Open it instead?`, 'Open', 'Cancel');
        if (overwrite === 'Open') {
    ...
    (0, output_channel_1.log)(FEATURE, `Created global doc: ${filePath}`);
    await openDoc(filePath);
}
/** Create a new doc inside a specific project's docs folder. */
async function newProjectDoc() {
    const registry = (0, registry_1.loadRegistry)();
    if (!registry) {
        return;
    }
    const projectPick = await vscode.window.showQuickPick(registry.projects.map(p => ({
        label: `$(folder) ${p.name}`,
        description: p.type,
        detail: p.path,
        project: p,
    ...
    fs.writeFileSync(filePath, content, 'utf8');
    (0, output_channel_1.log)(FEATURE, `Created project doc: ${filePath}`);
    await openDoc(filePath);
}
...
async function searchAllDocs() {
    ...
    const allFiles = listMarkdownFiles(registry.globalDocsPath)
...
function buildSyncIssues(project, globalDocsPath) {
    const issues = [];
...
function buildSyncHtml(results, registry) {
...
<div class="meta">
  Registry: <code>${esc(registry ? registry.registryPath ?? registry_1.REGISTRY_PATH : registry_1.REGISTRY_PATH)}</code><br>
...
async function syncCheck() {
    ...
    const results = registry.projects.map(project => {
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        ...
        try {
            switch (action) {
                ...
                case 'openRegistry': {
                    const doc = await vscode.workspace.openTextDocument(registry_1.REGISTRY_PATH);
                    await vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside);
                ...
                case 'createClaude': {
                    ...
                    (0, output_channel_1.log)(FEATURE, `Created CLAUDE.md: ${payload}`);
                    const doc = await vscode.workspace.openTextDocument(payload);
                    await vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside);
                ...
                case 'addGlobalRef': {
                    ...
                    (0, output_channel_1.log)(FEATURE, `Added global standards reference: ${payload}`);
                    const doc = await vscode.workspace.openTextDocument(payload);
                    await vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside);
                ...
                case 'createDocsFolder': {
                    ...
                    if (!fs.existsSync(payload)) {
                        ...
                        (0, output_channel_1.log)(FEATURE, `Created docs folder: ${payload}`);
                    }
                    ...
                    if (!fs.existsSync(statusFile)) {
                        ...
                        fs.writeFileSync(statusFile, `# CURRENT-STATUS.md — ${pName}\n\n---\n\n## 🅿️ PARKING LOT\n\n**Last action:** Initial setup\n**Next step:** TODO\n`, 'utf8');
                    }
                ...
                case 'fixAll': {
                    ...
                    const fresh2 = (0, registry_1.loadRegistry)();
                    if (!fresh2) {
                        break;
                    }
                    ...
                    (0, output_channel_1.log)(FEATURE, `Fixed ${fixed} issues for ${proj}`);
                    panel.webview.postMessage({ type: 'done', text: `Fixed ${fixed} issue${fixed !== 1 ? 's' : ''} in ${proj}` });
                    // Rescan
                    const fresh3 = (0, registry_1.loadRegistry)();
                    if (fresh3) {
            ...
            (0, output_channel_1.logError)(FEATURE, `syncCheck fix failed: ${action}`, err);
            panel.webview.postMessage({ type: 'error', text: String(err) });
        }
    });
    (0, output_channel_1.log)(FEATURE, `Sync check complete — ${registry.projects.length} projects checked`);
}
/** Open project-registry.json directly for manual editing. */
async function openRegistry() {
    await openDoc(registry_1.REGISTRY_PATH);
}
...
async function addProject() {
    ...
    (0, output_channel_1.log)(FEATURE, `Added project to registry: ${name.trim()} @ ${picked[0].fsPath}`);
}
// ─── Activate ────────────────────────────────────────────────────────────────
function activate(context) {
    ...
    vscode.window.onDidChangeActiveTextEditor(editor => {
        ...
        const panel = commands_1.getCatalogPanel && (0, commands_1.getCatalogPanel)();
        if (panel) {
            panel.webview.postMessage({ type: 'highlight', filePath });
        }
    });
    context.subscriptions.push(vscode.commands.registerCommand('cvs.docs.openGlobal', openGlobalDoc), vscode.commands.registerCommand('cvs.docs.openProject', openProjectDoc), vscode.commands.registerCommand('cvs.docs.newGlobal', newGlobalDoc), vscode.commands.registerCommand('cvs.docs.newProjectDoc', newProjectDoc), vscode.commands.registerCommand('cvs.docs.searchAll', searchAllDocs), vscode.commands.registerCommand('cvs.docs.syncCheck', syncCheck), vscode.commands.registerCommand('cvs.docs.openRegistry', openRegistry), vscode.commands.registerCommand('cvs.docs.addProject', addProject));
}
function deactivate() { }
