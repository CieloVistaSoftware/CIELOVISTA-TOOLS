...
exports.runLicenseSync = runLicenseSync;
/**
 * license-sync.ts
 *
...
function scanProject(project, canonical) {
...
// ─── HTML ─────────────────────────────────────────────────────────────────────
function buildHtml(projects, canonical) {
...
async function runLicenseSync() {
    ...
    if (!canonical) {
        const create = await vscode.window.showWarningMessage(`No canonical LICENSE found at ${CANONICAL_PATH}. Create one now?`, 'Create', 'Cancel');
        if (create !== 'Create') {
            return;
        }
        const year = new Date().getFullYear();
        ...
        (0, output_channel_1.log)(FEATURE, `Created canonical LICENSE at ${CANONICAL_PATH}`);
    }
    const projects = registry.projects.map(p => scanProject(p, canonical));
    const panel = vscode.window.createWebviewPanel('licenseSync', '📄 License Sync', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        ...
        try {
            ...
            if (action === 'saveCanonical') {
                ...
                (0, output_channel_1.log)(FEATURE, 'Canonical LICENSE saved');
            ...
            if (action === 'updateOne') {
                const proj = msg.proj;
                const canonical = msg.canonical.trim();
                const reg = (0, registry_1.loadRegistry)();
                if (!reg) {
                    return;
                }
                ...
                (0, output_channel_1.log)(FEATURE, `Updated LICENSE: ${licensePath}`);
            ...
            if (action === 'updateAll') {
                ...
                for (const p of reg.projects) {
                    ...
                    try {
                        ...
                        (0, output_channel_1.logError)(FEATURE, `Failed to write LICENSE for ${p.name}`, err);
                        skipped++;
                    }
                }
            ...
            if (action === 'diff') {
                ...
                await vscode.commands.executeCommand('vscode.diff', vscode.Uri.file(currentPath), vscode.Uri.file(CANONICAL_PATH), `${proj}/LICENSE ↔ Canonical`);
                return;
            }
            // ── Open ────────────────────────────────────────────────────
            if (action === 'open') {
            ...
            (0, output_channel_1.logError)(FEATURE, `License sync error: ${action}`, err);
            panel.webview.postMessage({ type: 'error', text: String(err) });
        }
    });
    (0, output_channel_1.log)(FEATURE, `License sync opened — ${projects.length} projects scanned`);
}
//# sourceMappingURL=license-sync.js.map