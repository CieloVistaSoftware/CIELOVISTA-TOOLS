...
function extractTitle(body, fileName) {
    ...
    if (h1) {
        return h1[1].trim().replace(/\*\*|__|\*|_|`/g, '');
    }
...
function extractDescription(body) {
    ...
    for (const line of lines) {
        ...
        if (t.startsWith('#')) {
            if (pastHeading && textLines.length) {
                break;
            }
            pastHeading = true;
            continue;
...
function extractTags(body, fileName) {
    const tags = new Set();
...
function buildRequiredFrontmatter(filePath, projectName, projectRoot, existingFm) {
    let content = '';
    try {
...
function applyHeader(filePath, projectName, projectRoot) {
    try {
        ...
        (0, output_channel_1.logError)(FEATURE, `Failed to apply header to ${filePath}`, err);
        return false;
    }
}
// ─── HTML report builder ──────────────────────────────────────────────────────
...
async function runScan() {
    ...
    _panel.webview.onDidReceiveMessage(async (msg) => {
        switch (msg.command) {
            case 'fixAll':
                ...
                break;
            case 'fixProject':
                await fixProject(msg.project);
                break;
            case 'fixFile':
                await fixSingleFile(msg.path, msg.project, msg.root);
                break;
            case 'open':
    ...
    const partial = reports.filter(r => r.hasFrontmatter && r.missingFields.length > 0).length;
    (0, output_channel_1.log)(FEATURE, `Scan: ${reports.length} docs — ${noHeader} no header, ${partial} partial`);
}
async function fixAll() {
    ...
    const confirm = await vscode.window.showWarningMessage(`Add/update headers in ${toFix.length} doc(s)? Existing content is never touched — only the frontmatter block is added or updated.`, { modal: true }, 'Fix All', 'Cancel');
    if (confirm !== 'Fix All') {
        return;
    }
    let fixed = 0;
    for (const report of toFix) {
        ...
        if (applyHeader(report.filePath, report.projectName, root)) {
            fixed++;
        }
    }
    _panel?.webview.postMessage({ type: 'done', text: `Updated headers in ${fixed} of ${toFix.length} docs. Rescanning…` });
    require('../shared/show-result-webview').showResultWebview('Doc Headers Updated', 'Fix All Doc Headers', 0, `Doc headers updated: <b>${fixed}</b> files.`);
    await runScan();
}
async function fixProject(projectName) {
    const registry = _registry ?? loadRegistry();
    if (!registry) {
        return;
    }
    ...
    let fixed = 0;
    for (const report of toFix) {
        if (applyHeader(report.filePath, projectName, root)) {
            fixed++;
        }
    }
    _panel?.webview.postMessage({ type: 'done', text: `Fixed ${fixed} docs in ${projectName}. Rescanning…` });
    require('../shared/show-result-webview').showResultWebview('Project Doc Headers Updated', `Fix Headers in ${projectName}`, 0, `Doc headers updated: <b>${fixed}</b> files in <b>${projectName}</b>.`);
    await runScan();
}
async function fixSingleFile(filePath, projectName, projectRoot) {
    if (applyHeader(filePath, projectName, projectRoot)) {
        (0, output_channel_1.log)(FEATURE, `Header applied: ${filePath}`);
    }
}
...
async function fixActiveFile() {
    ...
    if (applyHeader(filePath, projectName, projectRoot)) {
        require('../shared/show-result-webview').showResultWebview('Header Added/Updated', 'Fix Header in Current File', 0, `Header added/updated: <b>${path.basename(filePath)}</b>`);
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cvs.headers.fixAll', fixAll), vscode.commands.registerCommand('cvs.headers.fixOne', async () => {
        const registry = loadRegistry();
        if (!registry) {
            return;
        }
        ...
        if (picked) {
            await fixProject(picked.name);
        }
    }), vscode.commands.registerCommand('cvs.headers.fixFile', fixActiveFile), vscode.commands.registerCommand('cvs.headers.viewStandard', () => {
        vscode.window.showInformationMessage('Frontmatter standard: title, description, project, category, relativePath, created, updated, version, author, status, tags', 'Open Scan Panel').then(c => { if (c === 'Open Scan Panel') {
            runScan();
        } });
    }));
}
...
//# sourceMappingURL=doc-header.js.map