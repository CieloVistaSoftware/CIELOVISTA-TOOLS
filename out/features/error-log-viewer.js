...
exports.openErrorLogViewer = openErrorLogViewer;
exports.refreshErrorLogViewer = refreshErrorLogViewer;
/**
 * error-log-viewer.ts
 *
...
<div class="meta">Log file: <code>${esc((0, error_log_1.getLogPath)())}</code></div>
<script>
(function(){
...
async function openErrorLogViewer() {
    const errors = (0, error_log_1.getErrors)();
    ...
    _panel = vscode.window.createWebviewPanel('toolsErrorLog', '🪵 Tools Error Log', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    _panel.webview.html = html;
    _panel.onDidDispose(() => { _panel = undefined; });
    _panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.command === 'clear') {
            await (0, error_log_1.clearErrors)();
            _panel.webview.html = buildHtml([]);
            require('../shared/show-result-webview').showResultWebview('Error Log Cleared', 'Clear Error Log', 0, 'The error log has been <b>cleared</b>.');
        }
        if (msg.command === 'open-file') {
            const logPath = (0, error_log_1.getLogPath)();
            const doc = await vscode.workspace.openTextDocument(logPath);
            await vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside);
        }
    });
    (0, output_channel_1.log)(FEATURE, `Error log viewer opened — ${errors.length} entries`);
}
function refreshErrorLogViewer() {
    if (!_panel) {
        return;
    }
    _panel.webview.html = buildHtml((0, error_log_1.getErrors)());
}
//# sourceMappingURL=error-log-viewer.js.map