...
// ─── Compliance checker ───────────────────────────────────────────────────────
function normalizeHeading(h) { return h.toLowerCase().replace(/^#+\s*/, '').trim(); }
...
function checkCompliance(filePath, projectName, projectRoot) {
    ...
    try {
        ...
        return { filePath, fileName, projectName, readmeType, score: 0, issues: [{ severity: 'error', message: 'Could not read file', fixable: false, fixKey: '' }], lineCount: 0, missingRequiredSections: [], outOfOrderSections: [] };
    }
...
function applyFix(report) {
    ...
    for (const issue of report.issues) {
        ...
        if (issue.fixKey === 'feature-prefix') {
            content = content.replace(/^(#\s+)(.+)$/m, (_, hash, title) => title.toLowerCase().startsWith('feature:') ? `${hash}${title}` : `${hash}feature: ${title}`);
        }
...
let _diffPanel;
function buildDiffHtml(fileName, filePath, before, after, unifiedDiff, issues) {
    const fixableList = issues.filter(i => i.fixable).map(i => `<li>${i.severity === 'error' ? '🔴' : '🟡'} ${i.message}</li>`).join('');
...
async function showFixDiff(report) {
    ...
    const unifiedDiff = jsdiff.createPatch(report.fileName, before, after, 'before', 'after', { context: 4 });
    const html = buildDiffHtml(report.fileName, report.filePath, before, after, unifiedDiff, report.issues);
    if (_diffPanel) {
        ...
        _diffPanel = vscode.window.createWebviewPanel('readmeDiff', `📝 Fix — ${report.fileName}`, vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: false });
        _diffPanel.webview.html = html;
    ...
    _diffPanel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.command === 'approve') {
            try {
                ...
                (0, output_channel_1.log)(FEATURE, `Approved fix: ${report.filePath}`);
                _diffPanel?.dispose();
                // Count what changed
                ...
                (0, output_channel_1.logError)(FEATURE, `Failed to write fix for ${report.filePath}`, err);
                (0, result_viewer_1.showCommandResult)({
                    rc: 1,
                    commandId: 'cvs.readme.fix',
                    title: 'Fix a Non-Compliant README',
        ...
        if (msg.command === 'ignore') {
            (0, output_channel_1.log)(FEATURE, `Fix ignored: ${report.filePath}`);
            _diffPanel?.dispose();
            (0, result_viewer_1.showCommandResult)({
                rc: 2,
                commandId: 'cvs.readme.fix',
...
let _panel;
let _lastReports = [];
async function scanAllReadmes() {
    const registry = (0, registry_1.loadRegistry)();
    if (!registry) {
        return [];
    }
    return vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Scanning READMEs…', cancellable: false }, async (progress) => {
        const reports = [];
        const globalFiles = collectReadmes(registry.globalDocsPath, 'global', registry.globalDocsPath);
        for (const f of globalFiles) {
            reports.push(checkCompliance(f.filePath, f.projectName, f.projectRoot));
        }
        ...
        (0, output_channel_1.log)(FEATURE, `Scanned ${reports.length} READMEs`);
        return reports;
    });
}
async function runScan() {
    ...
    if (!reports.length) {
        vscode.window.showInformationMessage('No READMEs found.');
        return;
    }
...
function attachMessageHandler(panel) {
    panel.webview.onDidReceiveMessage(async (msg) => {
        switch (msg.command) {
            ...
            case 'aiFix':
                await aiFixReadme(msg.data, msg.readmeType);
                break;
            case 'fixProject':
...
async function pickAndFixReadme() {
    ...
    const picked = await vscode.window.showQuickPick(nonCompliant.map(r => ({
...
async function fixAllNonCompliant(reports) {
    ...
    const confirm = await vscode.window.showWarningMessage(`Write fixes to ${targets.length} README(s) without individual review? Each file will have missing sections added as stubs.`, { modal: true }, 'Write All', 'Cancel');
    if (confirm !== 'Write All') {
        return;
    }
    let fixed = 0;
    const changes = [];
    for (const report of targets) {
        try {
            ...
            (0, output_channel_1.logError)(FEATURE, `Failed to fix ${report.filePath}`, err);
        }
    }
    (0, result_viewer_1.showCommandResult)({ rc: 0, commandId: 'cvs.readme.fixAll', title: 'Fix All Non-Compliant READMEs', summary: `Fixed ${fixed} of ${targets.length} READMEs`, changes });
    await runScan();
}
async function aiFixReadme(filePath, readmeType) {
...
- Output ONLY the fixed markdown content, no preamble`;
    try {
        ...
        const fixed = await (0, anthropic_client_1.callClaude)(prompt, 3000);
        if (!fixed) {
        ...
        if (_diffPanel) {
            _diffPanel.title = `📝 AI Fix — ${report.fileName}`;
            _diffPanel.webview.html = html;
            _diffPanel.reveal(vscode.ViewColumn.Beside);
        }
        else {
        ...
        _diffPanel.webview.onDidReceiveMessage(async (msg2) => {
            if (msg2.command === 'approve') {
                ...
                _diffPanel?.dispose();
                (0, result_viewer_1.showCommandResult)({ rc: 0, commandId: 'cvs.readme.fix', title: 'Fix a Non-Compliant README (AI)', summary: `AI fix approved and written for ${report.fileName}` });
                await runScan();
            }
            if (msg2.command === 'ignore') {
                _diffPanel?.dispose();
                (0, result_viewer_1.showCommandResult)({ rc: 2, commandId: 'cvs.readme.fix', title: 'Fix a Non-Compliant README (AI)', summary: `AI fix ignored — no changes written to ${report.fileName}` });
            }
        });
        _panel?.webview.postMessage({ type: 'done', text: `AI diff ready for ${path.basename(filePath)} — approve or ignore` });
    }
    catch (err) {
        (0, output_channel_1.logError)(FEATURE, `AI fix failed for ${filePath}`, err);
        _panel?.webview.postMessage({ type: 'error', text: `AI fix failed: ${err}` });
    }
}
async function fixProjectReadmes(projectName) {
    const targets = _lastReports.filter(r => r.projectName === projectName && r.score < 80 && r.issues.some(i => i.fixable));
    if (!targets.length) {
        _panel?.webview.postMessage({ type: 'done', text: `No fixable issues in ${projectName}` });
        return;
    }
    let fixed = 0;
    for (const report of targets) {
...
async function createNewReadme() {
    const typePick = await vscode.window.showQuickPick([
        { label: '$(file) Project README', description: 'README.md at project root', type: 'PROJECT' },
        { label: '$(symbol-method) Feature README', description: 'myfeature.README.md inside a project', type: 'FEATURE' },
        { label: '$(book) Standard/Guide', description: 'In CieloVistaStandards/ or docs/', type: 'STANDARD' },
    ...
    (0, output_channel_1.log)(FEATURE, `Created new README: ${filePath}`);
}
// ─── Activate / Deactivate ────────────────────────────────────────────────────
function activate(context) {
    ...
    vscode.window.registerWebviewPanelSerializer('readmeCompliance', {
        async deserializeWebviewPanel(panel) {
            ...
            _panel.onDidDispose(() => { _panel = undefined; });
            attachMessageHandler(_panel);
            (0, output_channel_1.log)(FEATURE, 'README Compliance panel restored after reload');
        }
    });
    context.subscriptions.push(vscode.commands.registerCommand('cvs.readme.scan', runScan), vscode.commands.registerCommand('cvs.readme.fix', pickAndFixReadme), vscode.commands.registerCommand('cvs.readme.fixAll', async () => {
        const reports = _lastReports.length ? _lastReports : await scanAllReadmes();
        ...
        if (fs.existsSync(STANDARD_PATH)) {
            const doc = await vscode.workspace.openTextDocument(STANDARD_PATH);
            await vscode.window.showTextDocument(doc);
        }
        else {
            vscode.window.showErrorMessage(`Standard not found: ${STANDARD_PATH}`);
        }
...
//# sourceMappingURL=readme-compliance.js.map